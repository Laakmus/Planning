...
1. **Widoki listy a API** – Trzy zakładki (Aktualne, Zrealizowane, Anulowane) mapują się 1:1 na parametr `view` (CURRENT | COMPLETED | CANCELLED) w GET `/api/v1/orders`. Filtry i sortowanie współdzielone między zakładkami.
...
7. **Widok szczegółowy** – Jedna trasa/drawer; przy wejściu GET zlecenia + lock. Tryb tylko do odczytu przy cudzej blokadzie. Zapisz = PUT; status nie edytowany przez PUT (automatyczne WYS/KOR_WYS→KOR po stronie serwera).
...
14. **Status w liście** – Osobna kolumna; badge statusu w ciemniejszym kolorze, wiersz w jaśniejszym odcieniu tego samego koloru.
...
13. Status: osobna kolumna, ciemny badge, jasne tło wiersza.
...
- **Widok główny (lista)** – Zakładki nad tabelą; filtry (rodzaj transportu, przewoźnik, towar, miejsca, daty załadunku od/do, daty rozładunku od/do); wybór „Pokaż zleceń” (50/100/200) i wybór widoku listy (Widok 1 / Widok 2); tabela ze sticky header, zmienna wysokość wierszy; lewy klik → drawer szczegółów; prawy klik → menu (Wyślij maila, Historia zmian, Zmień status, Skopiuj zlecenie); ikona „Wyślij maila” w wierszu; przycisk „Dodaj nowy wiersz” (tylko w Aktualne). Stany puste: „Brak zleceń” / „Brak wyników dla filtrów” z przyciskiem wyczyść/dodaj.
...
- **Akcje:** POST `/orders/{id}/prepare-email` (Wyślij maila); POST `/orders/{id}/pdf` (Generuj PDF); POST `/orders/{id}/status`, POST `/orders/{id}/restore` (zmiana statusu, przywracanie). Sesja: Supabase Auth; GET `/auth/me` dla nazwy użytkownika w nagłówku.
